/* pinghreg.c
* programa cliente modbus TCP/IP para pingar o estado dos Holding Registers de um servidor
* Autor: Djan Rosario
...
int main(int argc, char *argv[]) {
    modbus_t *mb;
    int i = 0, j = 0;
    unsigned short porta = 502, limiteHRegs = 100, initAddr = 0;
    char *ip_servidor = "127.0.0.1";

    signal(SIGINT, sig_handler);

    if ((argc < 4) || (argc > 5)) {
        printf("Uso: %s <IP do Servidor Modbus> <Porta Servidor Modbus> <Offset> <Quantidade de HRegs lidos>\n", argv[0]);
        exit(1);
    }
    ...
    while(flag) {
        memset(tab_reg, 0, limiteHRegs);
        i = modbus_read_registers(mb, initAddr, limiteHRegs, tab_reg);
